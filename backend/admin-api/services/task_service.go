...
func (s *TaskService) GetTasksByUserId(ctx context.Context, userId string) ([]models.Task, error) {
	...
	if err != nil {
		s.logger.Error("Failed find tasks", zap.Error(err))
		return nil, err
	}
	return tasks, nil
}
...
func (s *TaskService) GetTaskById(ctx context.Context, taskID string) (*models.Task, error) {
	...
	if err != nil {
		s.logger.Error("Failed to parse task id", zap.Error(err))
		return nil, err
	}

	j, err := models.GetTaskFromCache(ctx, taskIDUint)
	if err != nil {
		s.logger.Error("Error while getting task from cache", zap.Error(err))
		return nil, err
	}
	if j != nil {
		return j, nil
	...
	if err != nil {
		s.logger.Error("Error while getting task from db", zap.Error(err))
		return nil, err
	}

	if err := models.SetTaskCache(ctx, task); err != nil {
		s.logger.Error("Error while setting task in cache", zap.Error(err))
		return nil, err
	}
...
func (s *TaskService) UpdateTask(ctx context.Context, task models.TaskDefinition, userID string, taskID string) (*models.Task, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			s.logger.Error("Task not found", zap.Uint64("task_id", taskIDUint))
