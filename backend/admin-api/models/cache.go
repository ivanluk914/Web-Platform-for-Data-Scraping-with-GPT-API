...
func SetTaskCache(ctx context.Context, task *Task) error {
	...
	return redisClient.Set(ctx, fmt.Sprintf("task:%d", task.ID), taskJSON, time.Hour).Err()
}
...
func GetTaskFromCache(ctx context.Context, taskID uint64) (*Task, error) {
